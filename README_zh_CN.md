...
**AE: 幻梦编码**（Fantasy Technology / 幻梦科技）为 NeoForge 1.21.1 上的 Applied Energistics 2 添加了一套紧凑的 ME 原生合成管线：**幻梦样板**（Fantasy Pattern）把处理配方以数据形式存入单个物品；**幻梦编码终端**（Fantasy Encoding Terminal）让你直接从配方查看器制作样板；**幻梦分子重构演算样板供应器**（Fantasy Molecular Reconfiguration Pattern Provider）方块则瞬间执行样板——从你的 ME 网络消耗原料、把产物直接注入网络，全程不需要任何中间机器。
...
### 🧩 幻梦样板
- 把处理配方编码进单个物品：最多 **81 个原料条目**、**6 个产物**，物品、流体、（装有 Mekanism 时）化学品皆可。
